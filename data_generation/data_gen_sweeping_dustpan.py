...
def data_gen_sweeping(info):
    ...
    if with_dustpan:
        ...
        dustpan_pos = np.array([-4.0, dustpan_pos_y, 0.5])
    ...
    pick_pos = dustpan_pos.copy()
    pick_pos_prev = [pick_pos.copy()]
    ...
    for p in range(n_push):
        ...
        pick_pos = dustpan_pos.copy()
        while pick_pos[0] < dustpan_pos[0] + 4.0 and np.abs(pick_pos[2] - pick_pos_prev[-1][2]) < 0.5:
            pick_id = rand_int(0, num_particle)
            pick_pos = particle_pos[pick_id, :3]
        # print(pick_pos)
        # print(np.abs(pick_pos[2] - pick_pos_prev[-1][2]))
        ...
        sponge_pos_end[0] += 0.8
        
        # pick the sponge start position according to the ending position and pick position
        # extrapolation
        slop = (sponge_pos_end[2] - pick_pos[2]) / (sponge_pos_end[0] - pick_pos[0])
        sponge_pos_x = pick_pos[0] + 6.0
        sponge_pos_z = slop * (sponge_pos_x - sponge_pos_end[0]) + sponge_pos_end[2]
