...
from data_generation.utils import calc_container_boxes
...
def data_gen_scooping(info):
    ...
    container, hide_shape_container = calc_container_boxes(
        pos=np.array([0., 0., 0.]),
        angle=0.,
    ...
    for i in range(len(container)):
        halfEdge = container[i][0]
        center = container[i][1]
        quat = container[i][2]
        hideShape = hide_shape_container[i]
        color = np.ones(3) * 0.9
    ...
    for p in range(n_scoop):
        ...
        for i in range(n_time_step):
            ...
            # set shape states
            shape_states = np.zeros((2 + len(container), 14))
            ...
            # set shape state for container
            offset = 2
            for idx_box in range(len(container)):
                shape_states[idx_box+offset, :3] = container[idx_box][1]
                shape_states[idx_box+offset, 3:6] = container[idx_box][1]
                shape_states[idx_box+offset, 6:10] = container[idx_box][2]
                shape_states[idx_box+offset, 10:] = container[idx_box][2]
