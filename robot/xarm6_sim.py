...
jointPositions= np.zeros(13) #[0,0,0,0,0,0]
rp = jointPositions
...
class XArm6Sim(object):
  def __init__(self, bullet_client, offset):
    ...
    orn=[0,0,0,1]
    self.xarm = self.bullet_client.loadURDF("../assets/xarm/xarm6_with_gripper_2.urdf", np.array([0,0,0])+self.offset, orn, useFixedBase=True, flags=flags)
    index = 0
  ...
  def step(self):
    ...
    if useIKFast:
      ...
      if useNullSpace:
        ...
        jointPoses.append(np.zeros(13))
        # print(len(jointPoses))
    #print("jointPoses=",jointPoses)
    if useDynamics:
      for i in range(xarmNumDofs-1):
